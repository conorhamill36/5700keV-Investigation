...
#import func_decorators as func_decs #Importing my own function decorators
#@func_decs.func_name
def plot_ang_dist(best_C2S_L1, best_C2S_L2, best_C2S_L3, l_1_array, l_2_array, l_3_array, total_array, target_4_com_angles_list, target_4_cs_list, target_4_cs_uncert_list, target_5_com_angles_list, target_5_cs_list, target_5_cs_uncert_list):
    print("Plotting ang distribution")
    plt.plot(np.linspace(0.0, len(l_1_array) - 1, num=len(l_1_array)) , best_C2S_L1 * np.array(l_1_array) )
    plt.plot(np.linspace(0.0, len(l_1_array) - 1, num=len(l_1_array)) ,best_C2S_L2 * np.array(l_2_array))
    plt.plot(np.linspace(0.0, len(l_1_array) - 1, num=len(l_1_array)) ,best_C2S_L3 * np.array(l_3_array))
    plt.plot(np.linspace(0.0, len(l_1_array) - 1, num=len(l_1_array)) ,total_array)
    ...
    plt.xlim(0,60)
    plt.ylim(0.08, 0.5)
    plt.ylabel(r'$\mathrm{d\sigma/d\Omega}$ (mb/sr)', fontsize=15, rotation=90, labelpad= 0)
    plt.xlabel(r'$\mathrm{\theta_{lab}[^{\circ}]}$', fontsize=15, labelpad=-2)
    plt.grid(b=False,which='minor',axis='y')
...
def add_image(fig):
    # im = Image.open('5.7_spectrum_sample.png')
    im = Image.open('run23_spectrum_zoomed.png')
    ...
    # # fig = plt.figure()
    # # im.set_size_inches(2, 2)
    # fig.figimage(im, xo = 100, yo = 50, zorder=10)
    # # fig.set_size_inches(20, 20)

    newax = fig.add_axes([0.15, 0.12, 0.3, 0.22], anchor='SW', zorder=10)
    newax.imshow(im)
    newax.axis('off')
...
def plot_single_state(E_x,J_pi,L_zero,L_one,L_two,L_three,S_0,S_1,S_2,S_3,counter, state_compound_factor):
    ...
    if(both_parities_boolean):
        ...
        with open("mg26dp_dwba_5.7_3-_1.dat") as l_1_file:
        ...
        with open("mg26dp_dwba_5.7_3-_3.dat") as l_3_file:
        ...
        if(positive_parity_boolean):
            ...
            print("Best reduced chi-squared of {} with sd {} found at:\nC2S(l=1): {}({})\nC2S(l=2): {}({})\nC2S(l=3): {}({})\n".format(best_reduced_chi_squared, best_reduced_chi_square_sd, best_C2S_L1*7.0, best_C2S_L1*7.0*0.2, best_C2S_L2*9.0, best_C2S_L2*9.0*0.2, best_C2S_L3*7.0, best_C2S_L3*7.0*0.2))
            ...
            #Making subplot with shared x axis
            # fig, (ax1, ax2) = plt.subplots(2, 0, sharex = 'all', sharey = 'all', gridspec_kw={'hspace': 0, 'wspace' : 0})
            # plt.subplot(2, 1, 2)


            # plot_ang_dist(best_C2S_L1, best_C2S_L2, best_C2S_L3, l_1_array, l_2_array, l_3_array, total_array, target_4_com_angles_list, target_4_cs_list, target_4_cs_uncert_list, target_5_com_angles_list, target_5_cs_list, target_5_cs_uncert_list)
            ...
            plt.errorbar(energy_diff_angle_array, energy_diff_array, energy_diff_uncert_array, \
            color='firebrick', barsabove=True, marker='.', ms=7, linestyle='None', elinewidth=1, capthick=1,\
            markeredgewidth=2)
            plt.ylabel(r'$\mathrm{\Delta}$E [keV]', fontsize=15,rotation=90, labelpad=-2)
            plt.xlim(0,60)
            ...
            print(len(l_1_array))
            print(len(best_C2S_L1 * np.array(l_1_array)))
            print(np.linspace(0.0, len(l_1_array) - 1, num=len(l_1_array) ) )
            ...
            plt.xlim(0, 60)
            plt.ylim(0.01, 1)
            plt.xlabel(r'$\mathrm{\theta_{lab}[^{\circ}]}$', fontsize=15)
            plt.ylabel(r'$\mathrm{d\sigma/d\Omega}$ (mb/sr)', fontsize=15,rotation=90, labelpad=-2)


            # plt.subplot(2, 1  , 2)
            plt.show()
            return #Temp return while making ang dist plot better
